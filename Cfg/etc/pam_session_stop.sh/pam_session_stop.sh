#!/bin/bash
# This script is called after other modules in the PAM session stack
# most notably after the pam_krb5. It can be used to execute commands
# immediately after the session is open.
...
if (( SESSION_COUNT == 0 )) && [[ "$PAM_TYPE" == "open_session" ]]; then
  logger "pam_session: first local user session is opened for $PAM_USER"

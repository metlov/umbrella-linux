...
class NagiosGen(Bcfg2.Server.Plugin.Plugin,
    ...
    def __init__(self, core, datastore):
        ...
        # KLM: this is a quick hack to enable Bcfg2 1.3.6 to handle the
        # local NagiosGen configuration. The proper solution
        # (via Genshi/Xinclude) is only possible in Bcfg2 1.4 .
        confpath=os.path.join(self.data,'..','Local','NagiosGen','config.xml')
        confpath=os.path.normpath(confpath)
        if not os.path.isfile(confpath):
            confpath=os.path.join(self.data,'config.xml')
        self.config = \
            Bcfg2.Server.Plugin.StructFile(confpath,

...
codenames_ubuntu=(precise trusty utopic bionic)
...
# GPG key
signing_key='metlov@fti.dn.ua'
signing_pass=''

# Debian files
debian_files_path="/home/metlov/temp/ossec"
...
do 
  ...
  do
    ...
    do
      ...
      cd ${source_path}
      if sudo DIST=$codename ARCH=$arch /usr/bin/pdebuild --use-pdebuild-internal --architecture ${arch} --buildresult ${results_dir} -- --basetgz \
      ...
      if [ -n "${signing_pass}" ] ;  then
        ...
        sudo debsign --re-sign -k${signing_key} ${results_dir}/${changes_file}
      fi
      if [ $? -eq 0 ] ; then
